...
class Player(wavelink.Player):
    ...
    async def add_tracks(self, search: str):
        search = search.strip('<>')
        if not URL_REG.match(search):
            search = f'ytsearch:{search}'

        try:
            tracks = await self.current_node.get_tracks(cls=wavelink.YouTubeTrack, query=search)
        except:
            tracks = await self.current_node.get_playlist(cls=wavelink.YouTubePlaylist, identifier=search)
    ...
    async def update_embed(self):
        content = '```Список очереди:\n' + '\n'.join(
            [f'{ind + 1}. {tr.title} ● {tr.requester.display_name}' for ind, tr in
             tuple(enumerate(self.queue._queue))[:15]]) + '```' if self.queue.qsize() > 0 else None
        await self.message_controller.edit(content=content,
                                           embed=self.build_embed())
